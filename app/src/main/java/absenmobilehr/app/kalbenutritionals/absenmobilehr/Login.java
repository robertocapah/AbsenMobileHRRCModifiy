...
//                            JSONObject jsn = jsonObject1.getJSONObject("validJson");
                        ...
                        if (result.equals("1")) {
                            ...
                            String strRoles = jsonObject1.getString("roleData");
                            String nameApp = null;
                            ...
                            if (strRoles != null && !strRoles.equals("null")){
                                JSONArray jArrayRoles = jsonObject1.getJSONArray("roleData");
                                if (jArrayRoles.length() > 0) {
                                    for (int a = 0; a < jArrayRoles.length(); a++) {
                                        JSONObject json_data = jArrayRoles.getJSONObject(a);
                                        int intRoleID = json_data.getInt("intRoleID");
                                        String txtRoleName = json_data.getString("RoleName");

                                        clsRole dataRole = new clsRole();
                                        dataRole.setIntRoleID(intRoleID);
                                        dataRole.setTxtRoleName(txtRoleName);
                                        try {
                                            new clsRoleRepo(getApplicationContext()).createOrUpdate(dataRole);
                                        } catch (SQLException e) {
                                            e.printStackTrace();
                                        }
                                    }
