...
import absenmobilehr.app.kalbenutritionals.absenmobilehr.Data.repo.clsRoleRepo;
...
import absenmobilehr.app.kalbenutritionals.absenmobilehr.Fragment.FragmentActivity;
...
import absenmobilehr.app.kalbenutritionals.absenmobilehr.Fragment.ProfilePurple;
...
//        }
        ...
        boolean isRC = false;
        try {
            List<clsRole> ltRole = new clsRoleRepo(getApplicationContext()).findAll();
            for (clsRole role : ltRole){
                if (role.getIntRoleID()==ROLE_RC){
                    isRC = true;
                }
            }
        } catch (SQLException e) {
        ...
        if (!isRC){
            header.removeItem(R.id.activity);
        }
...
//                                        task.execute();
                    ...
                    case R.id.testing:
                        ...
                        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                        ProfilePurple profilePurple = new ProfilePurple();
                        FragmentTransaction fragmentTransactionProfile = getSupportFragmentManager().beginTransaction();
                        fragmentTransactionProfile.replace(R.id.frame, profilePurple);
...
//                                        clsDeviceInfo dataDeviceInfoUser = new clsDeviceInfoRepo(context).getDataDevice(context);
                                        ...
                                        new VolleyUtils().makeJsonObjectRequestWithToken(MainMenu.this, strLinkAPI,token, mRequestBody, "Checking out please wait !", new VolleyResponseListener() {
                                            @Override
